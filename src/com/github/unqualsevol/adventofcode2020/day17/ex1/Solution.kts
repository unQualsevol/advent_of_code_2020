...
var pocketState = File("../input").readLines().mapIndexed { indexY, s ->
    s.mapIndexed { indexX, c -> Pair(Coordinate(indexX, indexY, 0), c) }
    ...
    for (z in -1..1) {
        for (x in -1..1) {
            for (y in -1..1) {
                ...
                val orDefault = previousState.getOrDefault(Coordinate(cube.x - x, cube.y - y, cube.z - z), inactive)
...
fun expandCube(cube: Coordinate, previousState: Set<Coordinate>): List<Coordinate> {
    ...
    for (x in -1..1) {
        for (y in -1..1) {
            for (z in -1..1) {
                val newCoordinate = Coordinate(cube.x - x, cube.y - y, cube.z - z)
